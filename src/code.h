...
typedef enum fr_code_storage_kind_t {
  FR_CODE_STORAGE_IMAGE = 0,
  FR_CODE_STORAGE_OVERLAY_RAM = 1,
} fr_code_storage_kind_t;

typedef struct fr_code_object_t {
  fr_code_storage_kind_t storage_kind;
  fr_code_offset_t instruction_byte_offset;
  uint16_t instruction_byte_length;
  fr_code_offset_t param_name_byte_offset;
...
typedef struct fr_code_table_t {
  ...
  uint8_t image_instruction_bytes[FR_PROFILE_TOTAL_IMAGE_BYTES > 0
                                      ? FR_PROFILE_TOTAL_IMAGE_BYTES
                                      : 1];
  uint8_t overlay_instruction_bytes[FR_PROFILE_MAX_OVERLAY_CODE_BYTES > 0
                                        ? FR_PROFILE_MAX_OVERLAY_CODE_BYTES
                                        : 1];
  char image_param_name_bytes[FR_PROFILE_MAX_PARAM_NAME_BYTES > 0
  ...
  char overlay_param_name_bytes[FR_PROFILE_MAX_OVERLAY_PARAM_NAME_BYTES > 0
                                    ...
                                    : 1];
  uint16_t image_used_instruction_bytes;
  uint16_t overlay_used_instruction_bytes;
  uint16_t image_used_param_name_bytes;
  uint16_t overlay_used_param_name_bytes;
  uint16_t base_image_used_instruction_bytes;
  uint16_t base_image_used_param_name_bytes;
  uint16_t count;
